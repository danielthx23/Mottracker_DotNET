...
namespace Mottracker.Presentation.Controllers
{
    ...
    public class TelefoneController : ControllerBase
    {
        ...
        public TelefoneController(ITelefoneUseCase useCase)
        {
            _useCase = useCase;
        }
        ...
        {
            var result = await _useCase.ObterTodosTelefonesAsync(Deslocamento, RegistrosRetornado);

            if (!result.IsSuccess) return StatusCode(result.StatusCode, result.Error);

            var hateaos = new
            {
                data = result.Value?.Data?.Select<TelefoneResponseDto, object>(t => new
                {
                    t.IdTelefone,
                    t.Numero,
                    t.Tipo,
        ...
        [SwaggerResponse(statusCode: 422, description: "Dados de entrada inválidos")]
        [SwaggerResponse(statusCode: 500, description: "Erro interno do servidor")]
        public async Task<IActionResult> GetByNumero(
            [FromRoute, SwaggerParameter("Número do telefone para busca", Required = true)] string numero)
        {
            var result = await _useCase.ObterTelefonePorNumeroAsync(numero);

            if (!result.IsSuccess) return StatusCode(result.StatusCode, result.Error);

            var hateaos = new
            {
                data = result.Value?.Select<TelefoneResponseDto, object>(t => new
                {
                    t.IdTelefone,
                    t.Numero,
                    t.Tipo,
        ...
        {
            var result = await _useCase.ObterTelefonePorUsuarioIdAsync(usuarioId);
        ...
        {
            var result = await _useCase.ObterTelefonePorTipoAsync(tipo);
        ...
        {
            var result = await _useCase.SalvarDadosTelefoneAsync(entity);
        ...
        {
            var result = await _useCase.EditarDadosTelefoneAsync(id, entity);
