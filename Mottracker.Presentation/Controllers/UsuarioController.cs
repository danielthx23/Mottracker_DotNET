...
namespace Mottracker.Presentation.Controllers
{
    ...
    public class UsuarioController : ControllerBase
    {
        ...
        public UsuarioController(IUsuarioUseCase useCase, IJwtService jwtService)
        {
            _useCase = useCase;
        ...
        [SwaggerOperation(
            ...
            Description = "Autentica um usuário no sistema usando e-mail e senha. " +
                        "Retorna os dados do usuário com token JWT de autenticação se as credenciais forem válidas. " +
                        "O token JWT deve ser enviado no header Authorization como 'Bearer {token}' para acessar endpoints protegidos."
        )]
            ...
            [FromBody, SwaggerParameter("Dados de login do usuário", Required = true)] UsuarioLoginDto loginDto)
        {
            ...
            var token = _jwtService.GenerateToken(
                result.Value.IdUsuario,
                result.Value.EmailUsuario,
                result.Value.NomeUsuario
            );
